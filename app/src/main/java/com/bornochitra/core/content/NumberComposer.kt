...
/**
 * Builds the guide of a two-digit number from its digits' guides (plan.md Steps 4 and 5), instead of
 * deriving each number from its own glyph. The ones digit follows the tens digit at the font's advance,
 * as the number is typeset (neither Inter nor Noto Sans Bengali kerns digits), and the pair is scaled
 * by [twoDigitScale] about the ink's vertical middle [inkCentreY] and centred on the canvas. Strokes
 * keep writing order — the tens digit, then the ones digit.
 *
 * Both values belong to the font: [twoDigitScale] is one size for all its two-digit numbers, the
 * largest at which its widest pair fits x 3..97, and [inkCentreY] is where the middle of its digits'
 * ink sits in the frame their guides are drawn in.
 *
 ...
 */
internal class NumberComposer(val twoDigitScale: Float, private val inkCentreY: Float) {
        ...
        fun DigitGuide.place(place: String, shift: Float) = strokes.map { stroke ->
            Stroke(
                ...
                points = stroke.points.map { point ->
                    Point(
                        x = CANVAS_CENTRE_X + (point.x + shift - inkCentreX) * twoDigitScale,
                        y = inkCentreY + (point.y - inkCentreY) * twoDigitScale,
                    )
                },
            )
        }
